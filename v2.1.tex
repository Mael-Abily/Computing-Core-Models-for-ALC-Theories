...
We can notice that all the variables are introduced by a Horn-$\mathcal{ALCH}$\todo{Create a macro for Horn-ALC.} axiom of the form \ref{rule:horn-alc-exist}.\todo{Note that $(3)$ is the only rule that features existentially quantified variables.}\todo{Use the commands ``href'' and ``label'' to reference items within the align environment.}
...
\end{itemize}
Consequently, $x$ is mergeable on $h(x)$ in $\Merge(G)$: contradiction\todo{which results on a contradiction}. So\todo{Hence,} $\Merge(G)$ is a core.
\end{proof}	
