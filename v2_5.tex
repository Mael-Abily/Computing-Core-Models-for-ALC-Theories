...
The conjunctive query entailment is an important issue in database\todo{knowledge representation}. This problem can be described in a first order logic background. We work on conjunctive formulas (that are formulas constructed only with conjunction and existential quantification) and queries that are conjunctive formulas without free variables; the answer of a query is either yes or no. For example $ \exists y. \textit{Human}(y) \wedge \textit{IsTheBrotherOf}(Pierre,y)$\todo{Use textit around Pierre.} is a query asking
...
We can now define the conjunctive query entailmen\todo{the problem of conjunctive query entailment}t: Given a set $T$ of existential rules and conjunctive formulas, and given a query $Q$ determine if the set $T$ entails the query $Q$. For example, the set $T= \{\textit{IsTheBrotherOf(Pierre,Marie)},$ $\forall x \forall y. \textit{IsTheBrotherOf}(x,y) \rightarrow \textit{Human}(x) \wedge \textit{Human}(y) \}$ entails the query $Q=\exists y. \textit{Human}(y) \wedge \textit{IsTheBrotherOf}(Pierre,y)$.



We usually use reasoning algorithms to answer the conjunctive query entailment. Note that in practice, a lot of formulas can be express\todo{expressed} via conjunctive formulas. By definition, a set $T$ of existential rules and conjunctive formulas entails a query $Q$ if every  model of $T$ is a model of $Q$. \todo{However, we cannot compute all models because...}It is not practical because a knowledge base often have an infinite number of models.

To deal with this problem, we can compute an\todo{a}\ universal model of the set $T$. That is a model of $T$ that is entailed by all the models of $T$. If a such model $U$ exists, we just need to show that $U$ entails $Q$ to conclude that $T$ entails $Q$. Hence, to solve query entailment, we just have to compute a finite universal model $U$ for a given input $T$ and look\todo{check}\ if $U$ entails $Q$.

To compute these models, we can use algorithms called the chase\todo{we can use the chase}. \todo{In this document, we present several variants of this algorithm:}We will present in this document the oblivious chase, the restricted chase and then the core chase. The last chase is the best in the sense of it\todo{it is the only one that} terminates if and only if there exists a finite universal model.

Nevertheless, it is the slowest\todo{I would just say that it is more complicated to implement.} so it it is never used for practical applications.\todo{Maybe explain a bit why is it hard to implement?} We will then focus on a restricted type of set $T$ where the set of conjunctive formulas is ground (that is there is no free-variable in the formula) and where the rules contained in $T$ are \ALCH\ axioms.\todo{Explain why \ALCH is interesting: talk a bit about description logics.} It is an interesting restriction because we can represent the results of the chase by a tree.\todo{I do not understand the intuitivion conveyed in this sentence.}

We will create a quicker chase for this restriction, that we called the merge chase, that would produce the same output than the core chase.\todo{We present a new variant of the chase for \ALCH theories which is also guaranteed to terminate if the input theory admits a finite universal model and is much easier to implement.} It is based on the idea that in a tree, there exists a sibling relation between the nodes (that is two nodes are siblings if they have the same father) and we will see that, in some conditions, a sibling of a node can be merged on this node like in the Figure \ref{figure:intro} where $y$ is a sibling of $t$ and is merged on it. We will then try to extend the merge chase on other type of knowledge bases.\todo{How to intuitiviely explain how the core chase is different from the merge chase: say that, in the core chase, one has to look for global endomorphisms in the sets of facts produced by the chase. In the merge chase, we only need to look for very local endomorphisms, which are much simpler to compute.}
...
Note that a ground factbase does not contain any  existential quantifiers.\todo{I would remove this sentence} In some articles, the factbases are always considered as ground but in this document, we consider factbases that may not be ground.\todo{Note that, we do consider factbases that are not ground, unlike other authors.} Consequently, \todo{the notions of Boolean conjunctive queries and factbases coincide}a boolean conjonctive query will be a factbase, so we will only talk about factbases and not introduce the notion of query.

For convenience, we identify factbases as sets of atoms, which allows us to  use  set  notions\todo{and operators from set theory}  such  as  set inclusion. For example, we identify the factbase $$\exists x,x_{1},x_{2},x_{3}. P(x) \land Q(x,a) \land R(x_{1},x_{2},x_{3},b)$$ with the set of facts $$\{P(x),Q(x,a),R(x_{1},x_{2},x_{3},b)\}$$\todo{You can save vertical space here.}
...
\begin{definition}
A factbase $F$ \emph{entails} another factbase $G$ (often noted $F \models G$) if each interpretation satisfying $F$ satisfies $G$. $F$ is equivalent\todo{Emphasize.}\ to $G$ if $F \models G$ and $G \models F$.
\end{definition}
...
This definition leads us to the central notion of Homomorphism\todo{lowercase}.
...
In practice, to know if $F \models G$, we use the following Theorem\todo{lowercase unless you add the href after.}.
...
The previous theorem has been proved in (\cite{base},theorem\todo{uppercase here} 6.2.3). For example, the factbase $F = \{P(b,a),A(x)\}$ entails the factbase $Q = \{P(x,a),P(y,z)\}$ due to the homomorphism $\{x \mapsto b, y \mapsto b, z \mapsto a \}$.
...
For a factbase $F$, let $id_{|F}$ be the substitution mapping each variable in $\Vars(F)$ to itself. And for a subsitution $\sigma$ defined on a factbase $G$ containing $F$, let $\sigma_{|F}$ be the subsitution\todo{maximal subset of} $\sigma$ defined only on $\Vars(F)$. In practice, it is better to work on smaller factbases.\todo{I do not understand the intuition conveyed by this sentence.} It leads us to the notions of retracts and cores:
...
If $G$ is a rectract of $F$, then $G$ is as strong\todo{logically strong} as $F$ and\todo{yet} smaller. We impose that $\sigma_{|G}=id_{|G}$ in the definition of retractations because it helps us in the proof of Proposition \ref{core} thanks to the well known property:
...
A core of a factbase $F$ is then one of the smaller\todo{smallest} retract of $F$:
...
\todo{Intuitivion for the core: the core of $F$ is one of the smallest sets of factst that is as strong as $F$.}
...
A factbase $M$ is a \emph{model} for a knowledge base $K = (R,F)$ if $M \models F$ and $M \models R$. The factbase $U$ is \emph{universal} for $K$ if for every model $M$ of $K$, $M \models U$. The factbase $M$ is an \emph{universal model} for $K$ if $M$ is a model for $K$ and $M$ is universal for $K$.\todo{Pull this sentence out of the definition and write: We often talk of universal models of a knowledge base $K$; that is, factbases that are both a model and universal for $K$.}
\end{definition}

For example, if we pose $K = (\{\alpha = A(x) \rightarrow \exists z.R(x,z) \wedge A(z)\},\{A(b)\})$, then an universal model for $K$ is $$U = \{A(b),R(b,x_0)\}\cup \{A(x_i)\mid i \in \N\}\cup \{R(x_i,x_{i+1}) \mid i \in \N\}$$ Note that the knowledge base $K$ does not admit finite universal models.\todo{I would put this in an example environment.}
...
An important problem that this document has\todo{that we want to solve is} to deal with is: Given a knowledge base $K=(R,F)$ and a factbase $Q$,  does $K \models Q$? It  is  well-known  that  this  problem  is  undecidable (\cite{NP2}, theorem 4). 
...
A sequence of rule applications is called a derivation. In this paper, we differentiate \todo{between} oblivious derivations and restricted derivations:
...
\begin{definition}
An \emph{oblivious chase} (resp. a restricted chase) for a knowledge base $K= (F,R)$ is a fair oblivious (resp. restricted) derivation $D=F_0,t_1,F_1,t_2,F_2,\ldots$ \todo{I would merge the previous 2 definitions.}
\end{definition}

For every oblivious chase $D = F_0,t_1,F_1,t_2,F_2,\ldots$ for $K$, we have $F_0 \subseteq F_1 \subseteq F_2 \subseteq \ldots$. T
, and the result of an oblivious chase does not depend on the derivation $D$ so we can pose \emph{$\textit{Obl(K)}$} = $\cup_{i \in \N}F_i$.\todo{Argue a bit why this is the case; one line suffices.} We say that the oblivious chase \emph{terminates} if $\textit{Obl(K)}$ is finite.

It is well known that:\todo{Remove one line paragraphs.}
...
The oblivious chase  can do a lot of applications that are useless: \todo{We present an example to showcase that the restricted chase may terminate even when the oblivivious chase does not:}
\begin{example}
...
\begin{definition}
If the core chase terminates on $K$ due to the finite core derivation $D=F_0,F_1,F_2,\ldots,F_i$, then we pose \emph{$\textit{C}(K)$}\todo{I would write $\textit{Core}(K)$.} = $F_i$. Otherwise, if the core chase does not terminate, $\textit{C}(K)$ is undefined.
\end{definition}
...
\item For every $i$, there exists $k \geq i$ such that $F_k$ is a core.
\end{itemize}
\todo{Do we really need to define fairness for atomic merge derivations? Discuss. -> I think it is necessary to prove that the atomic merge chase computes a universal model}
...
\end{enumerate}
We pose \emph{$\Fut(F_m,t,x) = F_m \cup h_{x/t}(\Tree_{F_m}(x))$}.\todo{This is a definition. Hence it should not be in a proposition environment.}
\end{proposition}
...
\end{itemize}
\todo{It would be good to give intuitive explanations for the above sequences of triggers.}
