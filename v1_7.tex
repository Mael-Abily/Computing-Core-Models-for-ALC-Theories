...
We only define what we need in first order logic but we do not redefine basics (interpretations, formulas,...\todo{Use \ldots instead of ...}).\todo{This sentence sounds a bit strange; rephrase.}
...
\begin{definition}
A \emph{factbase} $F$ is an existentially closed conjunction of atoms, that is\todo{Add a comma here.}\ a formula of the form $\exists x_{1},\ldots,x_{n}.P_{1}(t_{1}^{1},\ldots,t_{k_{1}}^{1})\land \ldots\land P_{m}(t_{1}^{m},\ldots,t_{k_{m}}^{m})$ where $t_i^j$ are terms and $P_i$ are predicates\todo{Add that this formula does not contain occurrences of free variables.}. A factbase is \emph{ground} if each of its atoms is ground.
\end{definition}

In the introduction, we talked about conjunctive formulas. We can show that a conjunctive formula is always equivalent to a factbase.\todo{What is the purpose of this sentence?}

In some articles, the factbase are always considered as ground but in this document, we consider factbases that may not be ground. Consequently, a  boolean conjonctive query will be a factbase, so we will only talk about factbases and not introduce the notion of query.\todo{Merge the last two paragraphs into one.}

For convenience, we identify factbases as sets of atoms, which allows\todo{add ``us'' here.}\ to  use  set  notions  such  as  set inclusion. For example, we identify the factbase $\exists x,x_{1},x_{2},x_{3}. P(x) \land Q(x,a) \land R(x_{1},x_{2},x_{3},b)$ with the set of facts $\{P(x),Q(x,a),R(x_{1},x_{2},x_{3},b)\}$.

For a formula $A$, let \emph{$\Vars(A)$} (respectively \emph{$\Csts(A)$}, and \emph{$\Terms(A)$}) be the set of variables (resp. constants, and terms) that occur in $A$.\todo{For a formula $A$, let \emph{$\Vars(A)$}, \emph{$\Csts(A)$}, and \emph{$\Terms(A)$} be the sets of variables, constants, and terms that occur in $A$, respectively.}

\subsubsection{Semantics}\todo{We should not introduce a subsubsection with a single definition; this document has a bit too much structure.}
...
\begin{definition}
For two factbases $F$ and $F'$, a \emph{homomorphism} from $F$ to $F'$ is a substitution $\sigma:var(F) \to term(F')$\todo{This is not the notation that was introduced in the previous subsubsection.}\ where $\sigma(F) \subseteq F'$. We say that a variable $x$ is \emph{map} to $\sigma(x)$.\todo{I do not understand this last sentence.}
\end{definition}

\begin{definition}
For two factbases $F$ and $F'$, an \emph{isomorphism} $h$ from $F$ to $F'$ is a bijective homomorphism where its inverse is an\todo{Write ``a'' instead of ``an''.}\ homomorphism from $F'$ to $F$.
\todo{Merge the last 2 def environments into 1.}
\end{definition}

\todo{Add ``For the remainder of this paper, ''.}We identify sets of facts that are unique up to isomorphism.
...
The following theorem has been proved in (\cite{base},theorem 6.2.3).\todo{Move this sentence after the theorem; the layout looks weird atm.}
...
\todo{This remark is not necessary, isn't it?}
\end{remark}
...
\begin{proposition}
$F'$ is a retract of $F$ if and only if $F' \subseteq F$  and there exists a retractation from $F$ to $F'$.\todo{The variables $F'$ and $F$ are not quantified in this proposition or the previous definition.}
\end{proposition}

\begin{definition}
If a factbase $F$ does not contain a strict retract\todo{The notion of a strict retract has not been introduced.}, then we say that $F$ is a \emph{core}. A \emph{core} of a factbase $F$ (noted \emph{$\textit{core}(F)$}) is a minimal retract of $F$ that is a core.
\end{definition}
...
\begin{example}
$F_1 = \{B(x,y),R(y,z)\}$ is the core of $F = \{B(x,y),R(y,z),B(x,w),R(w,z)\}$ because:\todo{Fix the margin.}
\begin{itemize}
\item $F_1 \subseteq F$;
...
\end{itemize}
\todo{Add ``Note that''}$F_2 = \{B(x,w),R(w,z)\}$ is also the core of $F$ and is indeed isomorphe\todo{isomorphic}\ to $F_1$ due to the homomorphism $\{x \mapsto x, y \mapsto w, z \mapsto z\}$ .
\end{example}
...
\todo{This section needs a bit of text throughout explaining things a bit. For instance, why are we interested in universal models? Explanation: these can be used to solve fact entailment.}
...
To determine how we apply a rule to a set of fact, we introduce the notion of trigger:\todo{Merge the previous paragraphs.}
...
\todo{You could say that $\sigma$ is a homomorphism from $Body(\alpha))$ to $F$.}
\end{itemize}
...
The oblivious (resp. restricted) derivation $D=F_0,t_1,F_1,t_2,F_2,\ldots$ is \emph{fair} if for every $i$ and every oblivious (resp. restricted) trigger $t$ applicable\todo{The notion of applicability has not been defined.}\ on $F_i$, there exists $k \geq i$ such that $\Appl(F_{k},t_k) = F_k$\todo{This may not be the case; a trigger can become satisfied w/o being fired.} (resp. $t$ is not anymore a restricted trigger on $F_k$).
\end{definition}

A fair derivation garantees that we consider every possible application. An easy way to have a fair derivation is to do a breadth-first search (BFS) on the terms.\todo{This sentence is unclear.}
...
\todo{The result of the restricted chase is not unique; hence, we cannot write $\textit{Res(O)}$. I thought I had left a comment about this problem.}
\end{definition}
...
\todo{We should have a result indicating that these two can be used to solve query entailment. Then we can explain that we can do so bc these are universal models.}
\end{theorem}
...
A \emph{core derivation} for a knowledge base $O = (R,F)$ is a (possibly infinite) sequence $D = F_0, F_1, F_2, \ldots$ where $F_0 = F$, and for $i >0$, either $F_{i}= \Appl(F_{i-1},t_i)$ is obtained by an application with $t_i$ a restricted trigger, or $F_i$ is obtained by computing the core of $F_{i-1}$ \todo{$F_i$ is the core of $F_{i-1}$}. 
\end{definition} 

\begin{definition}[Fairness]
A core derivation $D=F_0,F_1,F_2,\ldots$ is \emph{fair} if:
\begin{itemize}
\item For every $i$, for every restricted trigger $t$ applicable\todo{The notion of applicability has not been introduced.}\ on $F_i$, there exists $k > i$ such that $t$ is not anymore a restricted trigger on $F_k$.
...
\begin{definition}
A \emph{core chase} for a knowledge base $O= (R,F)$ is a fair core derivation $D=F_0,F_1,F_2,\ldots$ The core chase \emph{terminates} on $O$ if there exists a derivation $D=F_0,F_1,F_2,\ldots$ and $i \in \N$ such that there is not anymore any restricted trigger applicable on $F_i$\todo{There core chase terminates on $O$ if this knowledge base admits a finite core chase derivation.}. In this case, we pose \emph{$\textit{C}(O)$} = $F_i$.
...
\todo{Define $\textit{C}(O)$ after adding this remark. Also, add a citation to support it (look into ``The Chase Revisited'' paper.)}
...
\todo{Use the aling and/or itemize environments to improve the layout of this section.}
\end{example}
...
\todo{I understand that this is a paragraph to motivate our work. Perhaps it should go into the next section. That is, explain that compute the core of a fact set is computationally hard; then say that here we're presenting a more efficient way to solve this problem.}
