...
In this section, we only consider factbases with predicates of arity one or two. Hence, we will represent a factbase $F$ by a labelled graph $G = (V,E)$ where $V = \{t \mid t \in \Terms \}$ and $E = \{(t_1,t_2) \mid t_1,t_2 \in \Terms \wedge \Preds_F(t_1,t_2) \neq \emptyset\}$. We label the vertex $v \in V$ by exactly the predicates in $\Preds_F^1(v)$ and we label the edge between the terms $t_1$ and $t_2$ by exactly the predicates in $\Preds^2_F(t_1,t_2)$. For example, we represent $F = \{A(a), B(a),R(a,b),T(a,b),C(b),R(b,z)\}$ by the Figure \ref{figure:graph} \\
...
If a variable $x$ is mergeable on a term $t$ in a factbase $F$, then we note $h_{x/t}$ the substitution defined on $\Vars(F)$ such that for every variable $y$ in $\des(x)$, $h_{x/t}(y)$ is the variable $y$ where the occurence of $x$ is replaced by $t$, and for every variable $y$ not in $\des(x)$, $h(y) = y$. 

The \emph{atomic merging} of $x$ on $t$ in $F$ produces $h_{x/t}(F)$ 
\end{definition}


For example, in the Figure \ref{figure:atomic-merging}, $f_\alpha^y(a)$ is mergeable on $b$ in the left factbase. The atomic merging of $f_\alpha^y(a)$ on $b$ in this factbase gives the right factbase.
...
\begin{definition}
The atomic merge chase (resp. the merge chase) $D=F_0,F_1,F_2,\ldots$ \emph{terminates} on $O$ if it is a finite derivation. 
\end{definition}
...
\begin{definition}
For a term $t$ and a variable $x$ that occur in a merge chase derivation of some \ALCH\ knowledge base, we say that $t \prec x$ if $x$ is of the form $f_\alpha^y(t)$.
...
For a variable $x$ that occurs in a merge chase derivation of some \ALCH\ knowledge base, there exists exactly one predecessor for $\prec$.
\end{proposition}
...
For a term $t$ occurring in a chase derivation of the Horn-$ALC$ knowledge base $O$, we pose:
\begin{align*}
...
\begin{itemize}
\item $F_{m+1},\ldots,F_k$ have been obtained by a rule application,
...
To prove the result, we will look all the triggers dealing with the variables in $\des(x)$ and apply them on the variables in $\des(t)$ if the triggers have not already been applied.
...
\end{itemize} 
We note $h_{x/t}(tr_i)=(\alpha_i,h_{x/t} \circ \sigma_i)$. Let $tr_{k_1}',\ldots,tr_{k_r}'$ be all the triggers in $\{h_{x/t}(tr_1),\ldots,h_{x/t}(tr_n)\}$ such that 
\begin{itemize}
...
For a factbase $G$ obtained by applying some \ALCH\ axioms on $O$, $\Merge(G)$ is a core.
\end{proposition}
